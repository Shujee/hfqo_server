  ...
  <v-card class="ma-0 pa-0">
    ...
    <v-data-table
      ...
      <template v-slot:item.actions="{ item }">
        <v-icon @click="setExpiry(item)">mdi-clock</v-icon>
        <v-icon @click="deleteExam(item)">mdi-delete</v-icon>
      </template>
    </v-data-table>
  </v-card>
</template>
...
export default {
  ...
  methods: {
    deleteExam: async function(exam) {
      if (await this.$root.$confirm.open('Delete', 'Do you want to delete this master file?', { color: 'primary' })) {
